    ...
    override fun onDrawOver(canvas: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (colFreezeCount==1) {
            ...
            for (child in parent.children) {
                ...
                val rect = Rect(0, top, firstColWidth.value, top + recipeGrid[j][0].intrinsicHeight)
                ...
                // ## vertical divider
                defaultDividerDrawable.setBounds(firstColWidth.value, top, firstColWidth.value+defaultDividerHeight, top + recipeGrid[j][0].intrinsicHeight)
                defaultDividerDrawable.draw(canvas)
            }
        }
    }
