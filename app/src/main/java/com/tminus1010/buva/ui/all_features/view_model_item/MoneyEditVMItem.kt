...
class MoneyEditVMItem(
    ...
    override fun toViewItemRecipe(context: Context): IViewItemRecipe3 {
        return ViewItemRecipe3(context, ItemMoneyEditTextBinding::inflate) { vb ->
            fun setColor() {
                if (validation != null)
                    vb.moneyedittext.setTextColor(validation!!(vb.moneyedittext.easyText3?.toString()).toColor(vb.moneyedittext.context))
            }
            vb.moneyedittext.easyText3 = text1
            setColor()
            if (text2 != null) vb.moneyedittext.bind(text2) { if (text.toString() != it) easyText3 = it; setColor() }
